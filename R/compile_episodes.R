...
data %>% 
  dplyr::mutate(temp_end_date = if_else(is.na({{ end_date }}), as.Date("2099-12-31"), {{ end_date }}),
                dplyr::across(dplyr::where(is.POSIXct), as.Date), 
         datetimeend_num = as.numeric(temp_end_date)) %>%  #{{ end_date}})) # because cummax can't treat Date
    dplyr::arrange({{ personid }}, {{ start_date }}) %>%
    dplyr::group_by({{ personid }}) %>%
    dplyr::mutate(cummax_end = as.Date(cummax(datetimeend_num), origin = "1970-01-01"),
         new_episode_group = {{ start_date }} - dplyr::lag(cummax_end, default = cummax_end[1]) > lubridate::days(time_gap_threshold),
         episode = paste0({{ personid }}, "_", cumsum(new_episode_group) + 1),  
         temp_end_date = dplyr::if_else({{ end_date }} == "2099-12-31", NA_Date_, {{ end_date }})) %>%
    dplyr::group_by(episode) %>%
    dplyr::mutate(episode_start = min({{ start_date }}),
         episode_end   = max(temp_end_date)) %>%
    dplyr::ungroup() %>%
    dplyr::select(-c(datetimeend_num, cummax_end, new_episode_group, temp_end_date))
