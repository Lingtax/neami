#' Creating opinionated neami HTML from R Markdown
#'
#' This function is an alternative to the \code{\link[rmarkdown]{html_document}} formats to create HTML document
#' from R Markdown. It is almost verbatim prettydoc.  See the \href{https://prettydoc.statr.me/}{online documentation}
#' for more details.
#'
#' @param theme Character string to specify the document theme. Currently supported
#'              themes are \code{"neami"}.
...
#' @examples
#' \dontrun{
#' doc = system.file("rmarkdown", "templates", "html_neami_vignette",
#'                   "skeleton", "skeleton.Rmd", package = "neami")
#' rmarkdown::render(doc, html_neami(), output_dir = tempdir())
#' }
#' @export
#'
#'
html_neami <- function(theme = "neami",
                        highlight = NULL,
                        css = NULL,
  ...
  # designing new output format, and in fact html_neami() is largely based on
  # html_vignette() with some other features added in.
  #
  # The CSS used by html_neami() consists of three parts: the theme CSS, the
  ...
  css_dir  <- system.file("resources", "css",              package = "neami")
  hl_dir   <- system.file("resources", "css", "highlight", package = "neami")
  font_dir <- system.file("resources", "fonts",            package = "neami")
  img_dir  <- system.file("resources", "images",           package = "neami")
  js_dir   <- system.file("resources", "js",               package = "neami")
  tmpl_dir <- system.file("resources", "templates",        package = "neami")
  ...
  if (!isTRUE(theme %in% avail_themes)) {
    warning("theme not found, use default (neami) instead")
    theme <- "neami"
  }
  ...
  res <- rmarkdown::output_format(
    ...
    ## The real `theme` and `highlight` passed to html_neami() are
